...
namespace PSWindowsUpdate {
    ...
    [OutputType(new Type[] { typeof(WUJob) })]
    public class GetWUJob : PSCmdlet {
        ...
        private void CoreProcessing() {
            var invocationName = MyInvocation.InvocationName;
            if (string.Equals(invocationName.ToString(), "Clear-WUJob", StringComparison.OrdinalIgnoreCase)) {
                ClearExpired = true;
            }
            ...
            foreach (var text in computerName) {
                ...
                try {
                    ...
                    if (!MyInvocation.BoundParameters.ContainsKey("TaskName")) {
                        ...
                        pSObject2.Properties.Add(new PSNoteProperty("ComputerName", text));
                        switch (task.State) {
                            case _TASK_STATE.TASK_STATE_DISABLED:
                    ...
                    foreach (IRegisteredTask item2 in tasks2) {
                        if (item2.Definition.RegistrationInfo.Source == "PSWindowsUpdate" && DateTime.Parse(item2.Definition.Triggers[1].EndBoundary) < DateTime.Now &&
                            item2.State != _TASK_STATE.TASK_STATE_RUNNING && ShouldProcess(text, "Clear expired task: " + item2.Name)) {
                            folder.DeleteTask(item2.Name, 0);
                        }
                    }
                } catch {
                    var errorRecord2 = new ErrorRecord(new Exception("Cant find task " + TaskName + " on " + text), "WUJob", ErrorCategory.ResourceUnavailable, null);
